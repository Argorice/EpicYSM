...
public final class YsmRenderGate {
    ...
    private static <T extends Event> void listen(Class<T> type, Method playerOf) {
        MinecraftForge.EVENT_BUS.addListener(EventPriority.HIGHEST, false, type, event -> {
            try {
                if (playerOf.invoke(event) instanceof AbstractClientPlayer player
    ...
    private static boolean looksLikeRenderEvent(Class<?> type) {
        if (!type.isAnnotationPresent(net.minecraftforge.eventbus.api.Cancelable.class)) {
            return false;
        }
