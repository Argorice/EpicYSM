...
public final class YsmModelProbe {
    ...
    private static String runProbe(Object renderer, UUID playerId, ResourceLocation texture, Map<String, String> candidates) {
        ...
        if (renderer instanceof net.minecraft.client.renderer.entity.LivingEntityRenderer<?, ?> livingRenderer) {
            ...
            // The layers field is protected; NeoForge runs on official
            // mappings at runtime, so the reflective name is stable.
            try {
                Field layersField = net.minecraft.client.renderer.entity.LivingEntityRenderer.class.getDeclaredField("layers");

                if (layersField.trySetAccessible()) {
                    queue.add(new Node(layersField.get(livingRenderer), 1, false));
                }
